...
class SeekBarIndicated : FrameLayout, SeekBar.OnSeekBarChangeListener {
    ...
    private lateinit var textViewSeekBarMaxValue: TextView
    private lateinit var textViewSeekBarMinValue: TextView
    private lateinit var wrapperSeekBar: RelativeLayout
    private lateinit var wrapperSeekBarIndicator: RelativeLayout
    private lateinit var wrapperSeekBarMaxMinValues: RelativeLayout
    private lateinit var imageViewSeekBarIndicator: ImageView
    private lateinit var textViewIndicatorProgress: TextView
    ...
    private fun init(context: Context, attrs: AttributeSet? = null, defStyle: Int = 0) {
        val inflate = LayoutInflater.from(context).inflate(R.layout.view_seekbar_indicated, this)
        seekBar = inflate.findViewById<SeekBar>(R.id.seekBar)
        wrapperSeekBarMaxMinValues = inflate.findViewById<RelativeLayout>(R.id.wrapperSeekBarMaxMinValues)
        textViewSeekBarMaxValue = inflate.findViewById<TextView>(R.id.textViewSeekBarMaxValue)
        textViewSeekBarMinValue = inflate.findViewById<TextView>(R.id.textViewSeekBarMinValue)
        wrapperSeekBar = inflate.findViewById<RelativeLayout>(R.id.wrapperSeekBar)
        wrapperSeekBarIndicator= inflate.findViewById<RelativeLayout>(R.id.wrapperSeekBarIndicator)
        imageViewSeekBarIndicator= inflate.findViewById<ImageView>(R.id.imageViewSeekBarIndicator)
        textViewIndicatorProgress= inflate.findViewById<TextView>(R.id.textViewIndicatorProgress)
        btn= inflate.findViewById<TextView>(R.id.btn)
        if (attrs != null) {
    ...
    private fun updateIndicatorPosition() {
        ...
        btn.text = progress.toString()
            textViewIndicatorProgress.text = "Slide to choose your term$progress"
        wrapperSeekBarIndicator.viewTreeObserver.addOnGlobalLayoutListener(object : ViewTreeObserver.OnGlobalLayoutListener {
            override fun onGlobalLayout() {
                ...
                btn.x = thumbPos - wrapperSeekBarIndicator.width / 2.0f
    ...
    private fun setIndicatorImageAttrs(typedArray: TypedArray) {
        val imageResourceId = typedArray.getResourceId(R.styleable.SeekBarIndicated_indicator_src,
                R.drawable.bg_apply_loan_seek_tip)
